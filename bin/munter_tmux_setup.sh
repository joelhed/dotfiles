...
function create_named_window {
    tmux new-window -t $SESSION -a -n "$1" -c "$PROJECT_DIR/$1" vim
    tmux split-window -t $SESSION:$1 -c "$PROJECT_DIR/$1" -h -l 85
    tmux send-keys -t $SESSION:$1 "clear; ls" Enter
}
...
tmux send-keys -t $SESSION:0 "cd $PROJECT_DIR/deployment; clear; ls" Enter
